...
test.describe('Form Data Persistence', () => {
  test('Step 1: Personal Info persists after blur and refresh', async ({ page }) => {
  ...
  test('Step 2: Financial Info persists after blur and refresh', async ({ page }) => {
    await page.goto('/profile');
    await page.click('button:has-text("Next")');
    await page.waitForSelector('input[placeholder="50000"]', { timeout: 15000 });

    // Fill in Step 2 fields
    await page.fill('input[placeholder="50000"]', '75000');
    await page.fill('input[placeholder="10000"]', '25000');
    await page.selectOption('select', 'self-employed');
    ...
    expect(employment).toBe('self-employed');
  });
  ...
  test('Step 4: Career fields persist after blur and refresh', async ({ page }) => {
    ...
    await page.waitForSelector('input[placeholder="Software Engineer"]', { timeout: 15000 });
  ...
  test('Step 5: Family fields persist after blur and refresh', async ({ page }) => {
    ...
    await page.fill('input[placeholder="e.g., John Smith"]', 'Jane Doe');
    await page.selectOption('select:nth-of-type(2)', 'spouse');
    await page.fill('input[placeholder="e.g., US, UK, Germany"]', 'CA');

    // Blur fields to trigger save
    await page.locator('input[placeholder="e.g., John Smith"]').blur();
    await page.locator('input[placeholder="e.g., US, UK, Germany"]').blur();
    ...
    const familyName = await page.inputValue('input[placeholder="e.g., John Smith"]');
    const familyCitizenship = await page.inputValue('input[placeholder="e.g., US, UK, Germany"]');
  ...
  test('Step 6: Language fields persist after blur and refresh', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Add Language")', { timeout: 15000 });

    // Add a language
    await page.click('button:has-text("Add Language")');
    await page.waitForSelector('input[placeholder="English"]', { timeout: 15000 });
  ...
  test('Step 7: Country Selection fields persist after blur and refresh', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Add Country")', { timeout: 15000 });
